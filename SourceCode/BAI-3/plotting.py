...
# --- 2. Thống kê cụm ---
if 'Cluster' in pca_data.columns and 'Position' in pca_data.columns:
    total_players = len(pca_data)
    print(f"\nTong so cau thu: {total_players}")

    cluster_stats = pca_data.groupby('Cluster').agg(
        SoLuongCauThu=('Position', 'count'),
        ViTriPhoBien=('Position', lambda x: x.mode().iloc[0] if not x.mode().empty else 'Khong xac dinh')
    ).reset_index()

    print("\nThong ke theo cum:")
    for _, row in cluster_stats.iterrows():
        cluster_id = row['Cluster']
        count = row['SoLuongCauThu']
        pos = row['ViTriPhoBien']
        percentage = (count / total_players) * 100
        print(f"  Cum {cluster_id}: {count} cau thu ({percentage:.2f}%), phan lon choi o vi tri: {pos}")
else:
    print("\nKhong the thong ke vi khong tim thay cot 'Cluster' hoac 'Position' trong du lieu.")

# --- 3. Vẽ biểu đồ phân cụm 2D ---
print("\nBat dau ve bieu do phan cum 2D...")
